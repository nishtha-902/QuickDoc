...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Fetch doctor info for step 4
      const doctor = await Doctor.findById(DoctorId);

      // Format date and time for display
      const formattedDate = convert(AppointmentDate);
      const formattedTime = ConvertChosenTime(AppointmentDate);

      // Render appointmentbooking4 directly with data
      res.render('appointmentbooking4', {
        data: {
          doctor,
          date: formattedDate,
          time: formattedTime,
